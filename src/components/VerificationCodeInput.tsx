    ...
    (
        ...
        <div className={`w-full ${fieldClassName} group`}>
            {label && (
                <label
                    htmlFor={id}
                    className={`text-left block text-sm text-gray-700 mb-1 group-focus-within:text-blue-600 ${labelClassName} ${error ? 'text-red-600' : ''
                        }`}
                >
                    {label}
                </label>
            ...
            <div className={`flex gap-2 ${flexContainerClassName}`}>
                <input
                    ...
                    className={`rounded-3xl w-full h-11 px-4 border ${error ? 'border-red-500' : 'border-gray-300'
